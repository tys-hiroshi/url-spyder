...
#'http://link.springer.com/openurl?genre=book&isbn=978-3-662-56233-8'
    ...
    def parse(self, response):
        self.logger.error("======================response.url: " + response.url)
        print(response.url)
        contenturl = response.url.replace('book', 'content/pdf')
        contenturl = contenturl + '.pdf'
        self.logger.error("======================contenturl: " + contenturl)

        #webbrowser.open_new_tab(contenturl)

        # for item in response.xpath('//*[@id="main-content"]/article[1]/div/div/div[2]/div/div/a@href').extract():
        #     print("=========item" + item)
        yield scrapy.Request(contenturl, callback=self.save_pdf)
